...
3. ~~**Trailing slash**~~ — resolvido na infra: `Web::add()` e `Web::request()` normalizam ambos os lados (`'/' . trim($x, '/') . '/'`), então `/foo` e `/foo/` resolvem para a mesma rota. Convenção atualizada em `docs/conventions.md` (não registrar duas variantes).
4. **mTLS** — opcional pelo spec; o ambiente Apache deve aceitar conexão sem cert. Confirmar config no deploy real.
