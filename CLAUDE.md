...
- **QR Code:** `App\Core\QrCode` gera o símbolo (ISO 18004, modo byte, v1..40, L/M/Q/H) sem dependência alguma — nem Composer nem GD; o PNG é escrito na mão. Usado pelos `.../base64`, por `GET /pixqrcode/v2/{id}/imagem` e pela página `.../ver`. Foi conferido contra `qrencode` (matriz idêntica) e `zbarimg` (round-trip); como nenhuma das duas existe no servidor, o `tests/qrcode_smoke.php` decodifica a própria matriz em PHP puro. Se mexer no encoder, vale repetir a conferência externa — está descrita no cabeçalho do teste.
